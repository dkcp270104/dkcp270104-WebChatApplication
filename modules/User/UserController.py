...
def login():
    ...
    if (request.method == "POST"):
        ...
        user = UserModel.User(None, None, form_data.get(
            "userid"), form_data.get("password"), None, None, None, None, None, None, None, None)
...
def editprofile():
    ...
    if (request.method == "POST"):
        ...
        user = UserModel.User(userData[1].index, form_data.get("name"), userData[1].userid, form_data.get(
            "password"), userData[1].usertype, userData[1].avatar, form_data.get("bio"), form_data.get("country"), form_data.get("occupation"), form_data.get("DOB"), userData[1].verification, userData[1].messages)
        userService = UserServices.UserServices(db)
