...
class Repo():
    ...
    def addUser(self, user):
        try:
            query = """INSERT INTO "User" ( "name" ,"userid","password","usertype","avatar","bio","country","occupation","DOB","verification","messages") VALUES ('{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}');""".format(
                user.name, user.userid, user.password, user.usertype, user.avatar, user.bio, user.country, user.occupation, user.DOB, user.verification, json.dumps(user.messages))
            self.cur.execute(query)
            self.conn.commit()
        except Exception as e:
            print(e)
